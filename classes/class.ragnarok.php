<?php 
	...
	class ragnarok {
		...
		public function get_guild($gid = 0) {
			global $pdb;
			if($gid >= 1) {
			$name = iconv("cp1251", "utf-8", $pdb->Query("select `name` from `guild` where `guild_id`={$gid}"));
			$this->get_guild_emblem($gid);
			return "<img src=\"./guilds/{$gid}.png\" title=\"".msg_guild." - {$name}\" width=\"24\">";
			} else {
		...
		public function get_online() {
			global $pdb;
			$online = $pdb->Query("select count(*) from `char` where `online`='1'");
			return msg_online.$online;
		}
		...
		private function get_guild_emblem($gid=0) {
			global $pdb;
			if(!file_exists("./guilds/{$gid}.png") || ($_SERVER['REQUEST_TIME'] - emblem_lifetime) > filemtime("./guilds/{$gid}.png")) {
				$query_emblem = $pdb->Query("SELECT `emblem_data` FROM  guild WHERE `guild_id` = '".$gid."';");
				
				if($query_emblem != 0)
				{
					$query_emblem = gzuncompress(pack('H*',$query_emblem));
